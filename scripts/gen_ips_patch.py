...
from cu import read_ips_config
...
def run():
    ...
    id_to_patchlist = {}
    botwigd_offset = int(ips_config["botwigd_offset"], 16)
    fail_count = process_path(SRC, botwigd_offset, ips_config, id_to_patchlist)
    if fail_count > 0:
...
def process_path(path, botwigd_offset, ips_config, id_to_patchlist):
    """
    ...
    if isfile(path):
        if path.endswith(PATCH_EXTENSION):
            return 0 if compile_patch(path, botwigd_offset, ips_config, id_to_patchlist) else 1
        return 0
    ...
    if isdir(path):
        ...
        for sub_path in listdir(path):
            fail_count += process_path( \
                join(path, sub_path), botwigd_offset, ips_config, id_to_patchlist)
        return fail_count

    return 0

def compile_patch(path, botwigd_offset, ips_config, id_to_patchlist):
    """
        Compile the patch.
        ...
        path             File path
        botwigd_offset   The offset for botwigd relative to start of main,
    ...
    with open(path, "r", encoding="utf-8") as patch_file:
        for i, line in enumerate(patch_file):
            ...
            if line.startswith(".target "):
                ...
                botwigd_offset_from_target = botwigd_offset - target_offset
                continue
...
.asm, .bytes or .string to specify the type of the patch")
                return False
            patch_content = \
                produce_patch_content(patch_type, line, address, botwigd_offset_from_target)
...
def produce_patch_content(patch_type, line, address, botwigd_offset_from_target):
    ...
    if patch_type == PatchType.ASSEMBLY:
        return gen_asm_patch_bytes(address, line, botwigd_offset_from_target)
...
def gen_asm_patch_bytes(address, assembly_code_str, botwigd_offset_from_target):
    """
        ...
        botwigd_offset_from_target   Offset from target binary to botwigd
    """
    # asm patch
    ...
    if bl_match:
        ...
        branch_offset = resolve_bl_symbol(address,botwigd_offset_from_target, branch_symbol)
...
def resolve_bl_symbol(branch_address, botwigd_offset_from_target, branch_symbol):
    """
        ...
        branch_symbol                The symbol to resolve;
                                     can be a symbol in botwigd or address
    ...
    # If not, try searching the symbol in botwigd
    symbol_address = search_botwigd_symbol_addr(branch_symbol)
    if symbol_address is not None:
        target_address += symbol_address
        target_address += botwigd_offset_from_target
        return target_address - branch_address
...
def search_botwigd_symbol_addr(symbol):
    """
        Return the address of the symbol in botwigd, relative to start of botwigd
    ...
    if BOTW_IGD_SYMBOL_MAP is None:
        with open(join("build", "botwigd.map"), 'r', encoding="utf-8") as map_file:
    ...
    if pos == -1:
        print(f"Error: Cannot find botwigd symbol: {symbol}")
        return None
    # check uniqueness
    while True:
        ...
        if other_address != address:
            print(f"Error: Ambiguous botwigd symbol: {symbol}")
            return None
